...
public class BAT extends Plugin {
	...
	private static final int REQUIRED_BUNGEECORD_BUILD = 878;
	...
	public void onEnable() {
		...
		if (!ProxyServer.getInstance().getName().equals("BungeeCord")) {
			...
			getLogger().warning("BungeeCord version check disabled because a fork has been detected."
					+ " Make sur your fork is based on a BungeeCord build > #" + REQUIRED_BUNGEECORD_BUILD);

		} else if(getBCBuild() < REQUIRED_BUNGEECORD_BUILD && !new File(getDataFolder(), "skipversiontest").exists()) {

			getLogger().severe("Your BungeeCord build (#" + getBCBuild() + ") is not supported. Please use at least BungeeCord #" + REQUIRED_BUNGEECORD_BUILD);
		...
		if (config.isDebugMode()) {
			...
			try {
				...
				getLogger().info("The debug mode is now enabled ! Logs are available in debug.log file located in BAT folder");
		...
		loadDB(new Callback<Boolean>() {
			...
			public void done(final Boolean dbState, Throwable throwable) {
				if (dbState) {
					...
					getLogger().severe("BAT must shutdown because it can't connect to the database.");
					return;
				}
	...
	/**
	 * @return The current BungeeCord build
	 */
	public int getBCBuild() {
		...
		try {
			...
			BCBuild = REQUIRED_BUNGEECORD_BUILD;
		}
	...
	public void loadDB(final Callback<Boolean> dbState) {
		...
		// (if it isn't already in the lib folder) and load it
			...
			getLogger().warning("It is not recommended to use SQLite with BAT,"
	...
	/**
	 * Get plugin instance
	 * @return Instance of main plugin class
	 */
	...
	/**
	 * This method appears to prepare a message for dispay with correct prefix and colour codes
	...
	/**
	 * @return The modules manager of the plugin
	 */
	...
	/**
	 * @return The main config file of the plugin
	 */
